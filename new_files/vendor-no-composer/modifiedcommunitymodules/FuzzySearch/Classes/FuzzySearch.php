...
namespace ModifiedCommunityModules\FuzzySearch\Classes;
      ...
      if (MODULE_MCM_FUZZY_SEARCH_ENABLE_PROXIMITY_COLOR == 'true') {
        ...
        while ($results['name'][$counter] && $counter < MODULE_MCM_FUZZY_SEARCH_MAX_KEXWORD_SUGGESTS) {
          ...
          if (MODULE_MCM_FUZZY_SEARCH_COUNT_PRODUCTS == 'true') {
            ...
            $products_count_query = xtc_db_query("SELECT COUNT(*) as 'products_count' FROM " . TABLE_PRODUCTS . " AS p
          ...
          // ins Array
          $this->resultKeywords[$counter] = [
            'SUGGEST_KEYWORD' => $results['name'][$counter],
          ...
          while ($products_suggest = xtc_db_fetch_array($products_suggest_query)) {
            ...
            if (($counter <= MODULE_MCM_FUZZY_SEARCH_MAX_PRODUCTS_SUGGEST) && ($results['name'][$row]) && (!$id_in_array)) {
              ...
              if (SHOW_SHIPPING == 'true') {
                $ship_info = ' ' . SHIPPING_EXCL .
                              '<a href="javascript:newWin=void(window.open(\'' . xtc_href_link(FILENAME_POPUP_CONTENT, 'coID='.SHIPPING_INFOS) . '\', \'popup\', \'toolbar=0, width=640, height=600\'))"> ' . SHIPPING_COSTS . '</a>';
              }
     
              $this->resultProducts[$counter] = [
                'PRODUCTS_NAME' => $products_suggest['products_name'],
                ...
                'PRODUCTS_IMAGE' => is_file(DIR_WS_THUMBNAIL_IMAGES . xtc_get_products_image($products_suggest['products_id']))
                                    ? DIR_WS_THUMBNAIL_IMAGES . xtc_get_products_image($products_suggest['products_id'])
                                    : DIR_WS_THUMBNAIL_IMAGES . 'noimage.gif'
              ];
            }
            $counter++;
          }
